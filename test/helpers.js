  ...
  return [
    {
      ...
      profile_img_link:
        "https://pmcdeadline2.files.wordpress.com/2019/12/orlando-bloom.jpg?w=1000",
...
function makeExpectedMessage(users, message) {
  const curator = users.find((user) => user.id === message.curator_id);
...
function makeExpectedSubscriber(users, subscriber) {
  const curator = users.find((user) => user.id === subscriber.curator_id);
